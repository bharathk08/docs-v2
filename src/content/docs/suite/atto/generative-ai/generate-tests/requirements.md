...
description: "In Testsigma, you can create test cases from Jira stories & epics by integrating Jira | Learn how to create test cases from Jira using GenAI capabilities"
sidebar:
  order: 4.7421
---
...
:::note[Troubleshooting: Figma Pages Not Loading]
...
**Cause**: Figma applies rate limits to REST API requests based on the user’s seat type and access level. When these limits are reached, Figma temporarily stops returning page/file information to external tools. Users with lower access seats are more likely to reach these limits.

**Steps to resolve:**

1. **Verify the Figma API Key Permissions** — Ensure the API key used in Testsigma belongs to a Figma account with a **Dev/Full seat**. These roles provide higher API rate limits and help prevent intermittent failures.
2. **Re-generate the API Key (If Needed)** — If the current key was created under a seat with restricted API access, generate a new key from an account with a higher access tier.
3. **Wait for Rate Limits to Reset** — If rate limits were exceeded, Figma may temporarily block further requests. Wait a few minutes and then try loading the pages again.
4. **Review Figma Rate Limits** — For detailed info on rate limits, see [Figma’s Rate Limits](https://developers.figma.com/docs/rest-api/rate-limits/).
:::
...
3. In the **Generate Test Cases** section, select **Live Recorder** as an input source.
   ![Live Recorder Input Source](https://s3.amazonaws.com/website-static-docs.testsigma.com/new_images/projects/Updated_Doc_Images/Live_Recorder_Input_Source.png)

4. In the **Live Recorder** dialog, click **Start Recording**. A new Chrome session opens automatically.
...
5. Enter the URL to test and click **Start Recording**.
...
6. When prompted to choose what to share, select **Entire screen** or **Window** as needed.
   ![Screen Type Selection](https://s3.amazonaws.com/website-static-docs.testsigma.com/new_images/projects/Updated_Doc_Images/screen_type_selection.png)
...
8. Click **Stop**, review the recorded video, and click **Continue**.
   ![Stop Live Recording](https://s3.amazonaws.com/website-static-docs.testsigma.com/new_images/projects/Updated_Doc_Images/Stop_Live_Recording_in_GA.png)
   
9. Review the recording again and click **Continue**.
   ![Continue](https://s3.amazonaws.com/website-static-docs.testsigma.com/new_images/projects/Updated_Doc_Images/Watch_and_Continue_with_Video_recorded.png)

10. You’ll be redirected to Testsigma with the **Live Recorder** dialog displaying your recording.

11. Click **Save** to add the recording as context for test case generation.
